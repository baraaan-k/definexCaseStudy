...
class ThirdCellView: UICollectionViewCell {
    static let identifier = "ThirdCellView"
    ...
    private let starView: CosmosView = {
        ...
        starView.rating = 4
        starView.settings.starSize = 16
        starView.settings.filledColor = .green
        starView.settings.filledBorderColor = .green
        starView.settings.emptyColor = .lightGray
        starView.settings.emptyBorderColor = .lightGray
        return starView
    }()
    ...
    override init(frame: CGRect) {
        ...
        let stackView = UIStackView(arrangedSubviews: [imageView, titleLabel, priceLabel, discountStackView, starView])
    ...
    func configure(with product: DiscoverListResponse) {
        ...
        if let ratePercentage = product.ratePercentage {
            let rate = getStarRating(from: ratePercentage)
    ...
    func getStarRating(from score: Int) -> Int {
        let starPerPoint = 20
        let stars = score / starPerPoint
        return min(stars, 5)
    }
}
